...
### Alterado
- A descoberta individual de refresh/expiry das zonas secundárias passa a ser opt-in via `{$BIND.ZONE.SECONDARY.MATCHES}`; o padrão `^# Histórico de alterações
...
- Métricas de memória do BIND a partir do endpoint nativo de memória.
 ...
 não cria itens individuais por zona secundária.
